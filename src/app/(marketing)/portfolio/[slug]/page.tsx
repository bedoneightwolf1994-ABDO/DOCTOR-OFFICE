  ...
  return (
    <div className="container-page section max-w-4xl">
      ...
      {p.featured_image_url && (
        ...
        <img src={p.featured_image_url} alt={p.title} className="w-full rounded-lg mt-8 aspect-video object-cover" />
      )}
      ...
      {p.additional_images?.length > 0 && (
        <div className="grid grid-cols-2 gap-4 mt-8">
          {p.additional_images.map((img: string, i: number) => (
            // eslint-disable-next-line @next/next/no-img-element
            <img key={i} src={img} alt={`${p.title} ${i + 1}`} className="rounded-lg w-full aspect-video object-cover" />
          ))}
        </div>
      )}
